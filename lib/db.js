...
function filterModels(models, filterOptions) {
  _.each(filterOptions, function(filter, key) {
    if(key === 'in') {
      models = _.filter(models, function(model) {
        var filterKey = _.keys(filter)[0]; // currently support only single 'in' filter
        var valueToSearch = filter[filterKey];
        return model.hasOwnProperty(filterKey)
          && _.contains(model[filterKey], valueToSearch);
      });
      delete filterOptions['in'];
    }
  });
